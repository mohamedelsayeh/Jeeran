...
import UIKit
class SubServices: UIViewController,UIScrollViewDelegate, UITableViewDelegate,UITableViewDataSource {
    @IBOutlet weak var tableView: UITableView!
    
    @IBOutlet weak var scrollView: UIScrollView!
    var imageNames = [String]()
    var subServicesCategory = [ResponseSubServices]()
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if(self.subServicesCategory.count==0)
        {return 0;}
        else
        {return self.subServicesCategory.count;}
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
         ...
         if(self.subServicesCategory.count==0)
        {
            ...
            WebserviceManager.getImage( self.subServicesCategory[indexPath.row].logo! , result: { (image, code) in
                cell.imageCell.image = image
            })
             cell.cellName.text = self.subServicesCategory[indexPath.row].title_en!
            }
        return cell
    }
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        self.performSegueWithIdentifier("detailsServices", sender: self)
             //        let subService = SubServices()
        //   subService.main_Service_id =
        //        self.navigationController?.pushViewController(subService, animated: false)
    }
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        
        if segue.identifier=="detailsServices" {
            let detailService = segue.destinationViewController as! ServicePlaceList
            detailService.serviceName = serviceName
            detailService.main_Service_id = self.sub_Service_id
               print("gggggggggggggggg=========>>>>>",   detailService.main_Service_id)
